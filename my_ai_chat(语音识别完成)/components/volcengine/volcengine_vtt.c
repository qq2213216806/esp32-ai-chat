...
static Header_Type generate_header( uint8_t version,
...
void  construct_full_json(char *out_string)
{    
    uuid_t uu;
...
int parse_response_mes(char *mes,Mes_Type *response_mes)
{
    ...
    }else if(Get_Header_Message_type(header) == SERVER_ACK)
    {
        printf("SERVER_ACK\n");
        error_code = 1024;
...
int parse_json(char *json,char *response_text,int *sequence)
{
   ...
   // esp_websocket_client_handle_t client = (esp_websocket_client_handle_t)msg->Wed_client;
    ...
    if (msg->event_id == WEDSTREAM_EVENT_DATA_AUDIO_WRITE)
    {
        ...
        if( vtt->is_begin == true)
        {
            ...
            construct_full_json(vtt->send_buffer+8);     //paylaod消息体,从第8个字节位置写入
